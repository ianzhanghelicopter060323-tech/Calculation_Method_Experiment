function [y0] = neville_eval(X, Y, x0)
    ...
    for i=1:(m-1) % outer loop stands for layers
        ...
        for j=(i+1):m % inner loop stands for each interval in this layer
            ...
            w = (x0 - X(j))/(X(leftmost) - X(j)); % define interpolation opertater

            P(j) = P_former(j)+(P_former(j-1)-P_former(j)) * w;
            % Not the same Neville Interpolation formula as the boy given
            % P(j) = ((x0 - X(leftmost))*P_former(j) - (x0 - X(j))*P_former(j-1)) / (X(j) - X(leftmost));
        end
